...
router.get('/zones', auth, adminOnly, (req, res) => {
  try {
    const zones = db.prepare('SELECT * FROM shipping_zones ORDER BY created_at DESC').all();
    res.json({ zones });
  } catch (err) {
...
router.post('/zones', auth, adminOnly, (req, res) => {
...
router.put('/zones/:id', auth, adminOnly, (req, res) => {
...
router.delete('/zones/:id', auth, adminOnly, (req, res) => {
  try {
