...
router.get('/', auth, adminOnly, cacheMiddleware(() => 'admin:stats', 60000), (req, res) => {
  try {
    const { start, end } = req.query;
    let dateFilter = '';
    const params = [];
    if (start) { dateFilter += ' AND created_at >= ?'; params.push(start); }
    if (end) { dateFilter += ' AND created_at <= ?'; params.push(end); }
