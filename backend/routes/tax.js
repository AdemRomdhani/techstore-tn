...
router.get('/', auth, adminOnly, (req, res) => {
  try {
    const rates = db.prepare('SELECT * FROM tax_rates ORDER BY created_at DESC').all();
    res.json({ rates });
  } catch (err) {
...
router.post('/', auth, adminOnly, (req, res) => {
  ...
  try {
    ...
    const taxRate = db.prepare('SELECT * FROM tax_rates WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', auth, adminOnly, (req, res) => {
  ...
  try {
    ...
    const taxRate = db.prepare('SELECT * FROM tax_rates WHERE id = ?').get(req.params.id);
    res.json({ taxRate });
  } catch (err) {
